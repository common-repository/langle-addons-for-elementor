...
trait Button_Renderer {
	...
	public function render_icon_button( $args = [] ) {
		$args = wp_parse_args( $args, [
			...
			'text'       => 'button_text',
			'link'       => 'button_link',
			'class'      => 'la-btn la-btn--link',
			'text_class' => 'la-btn-text',
		] );
		...
		elseif ( empty( $button_text ) && ( ! empty( $has_old_icon ) || ! empty( $has_new_icon ) ) ) : ?>
			<a <?php $this->print_render_attribute_string( 'button' ); ?>><?php langle_addons_render_button_icon( $settings, $args['old_icon'], $args['new_icon'] ); ?></a>
		<?php elseif ( $button_text && ( ! empty( $has_old_icon ) || ! empty( $has_new_icon ) ) ) :
			if ( $settings[ $args['icon_pos'] ] === 'before' ) :
				$this->add_render_attribute( 'button', 'class', 'la-btn--icon-before' );
				$button_text = sprintf( '<span %1$s>%2$s</span>', $this->get_render_attribute_string( $args['text'] ), esc_html( $button_text ) );
				?>
				<a <?php $this->print_render_attribute_string( 'button' ); ?>><?php langle_addons_render_button_icon( $settings, $args['old_icon'], $args['new_icon'], ['class' => 'la-btn-icon'] ); ?> <?php echo esc_html( $button_text ); ?></a>
				<?php
			else :
				$this->add_render_attribute( 'button', 'class', 'la-btn--icon-after' );
				$button_text = sprintf( '<span %1$s>%2$s</span>', $this->get_render_attribute_string( $args['text'] ), esc_html( $button_text ) );
				?>
				<a <?php $this->print_render_attribute_string( 'button' ); ?>><?php echo esc_html( $button_text ); ?> <?php langle_addons_render_button_icon( $settings, $args['old_icon'], $args['new_icon'], ['class' => 'la-btn-icon'] ); ?></a>
				<?php
			endif;
		endif;
	}
